...
const getBME280 = async () => {
  ...
  const dig_H2 =
    (dig_h.buffer.readInt16LE(1) << 8) | dig_h.buffer.readInt16LE(0);
  const dig_H3 = dig_h.buffer.readInt8(2);
  const dig_H4 =
    (dig_h.buffer.readInt16LE(3) << 4) | (dig_h.buffer.readInt16LE(4) & 0x0f);
  const dig_H5 =
    (dig_h.buffer.readInt16LE(5) << 4) |
    ((dig_h.buffer.readInt16LE(4) >> 4) & 0x0f);
  ...
  v1 =
    ((dig_P3 * (((v1 / 4.0) * (v1 / 4.0)) / 8192)) / 8 + (dig_P2 * v1) / 2.0) /
    262144;
  ...
  let pressure = (1048576 - adc_P - v2 / 4096) * 3125;
...
port.on("data", (data) => {
  ...
  if (match) {
    const co2ppm = parseInt(match.toString().match(/\d+/));
    handleS300L(co2ppm);
...
let dataLog = [];
async function handleS300L(co2) {
  const { temperature, humidity, pressure } = await getBME280();

  const now = new Date();
  if (now.getMinutes() != lastSend.getMinutes()) {
    // console.log(now.toISOString());
    // console.log(`samples: ${dataLog.length}`);
    if (dataLog.length > 18) {
      const sum = dataLog.reduce(
        (acc, v) =>
          Object.fromEntries(
            Object.entries(acc).map(([key, value]) => [key, value + v[key]])
          ),
        {
          temperature: 0,
          humidity: 0,
      ...
      const ave = Object.fromEntries(
        Object.entries(sum).map(([key, value]) => [key, value / dataLog.length])
      );
      // console.log(`Temperature: ${ave.temperature} C`);
      // console.log(`Humidity: ${ave.humidity} %`);
      // console.log(`Pressure: ${ave.pressure} hPa `);
      // console.log(`CO2: ${ave.co2} ppm`);
      postData({ datetime: lastSend.toISOString(), ...ave });
    }
    lastSend = now;
    dataLog = [];
  }
